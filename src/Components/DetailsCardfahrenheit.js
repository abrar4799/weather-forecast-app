...
function DetailsCardfahrenheit({weather_icon , weather_data}){
    const { clouds , main , weather } = weather_data.list[0]

    return (
        <div  className="container p-4 flex items-center justify-center shadow-lg rounded-lg bg-white h-1/3 mb-auto">
        <div className="my-auto">
        <p className="font-bold text-5xl text-pink-800 mb-2">{Math.round(main.temp)* 9 / 5 + 32}&deg;F</p>
        ...
        <div className="my-2 border-l-2 border-gray-100 p-2">
            <p className="text-gray-400 text-lg">RealFeel: {Math.round(main.feels_like)* 9 / 5 + 32}&deg;F</p>
            <p className="text-gray-400 text-lg">Humidity: {main.humidity}%</p>
            <p className="text-gray-400 text-lg">Cloud Cover: {clouds.all}%</p>
            <p className="text-gray-400 text-lg">Min Temp: {Math.round(main.temp_min)* 9 / 5 + 32}&deg;F</p>
            <p className="text-gray-400 text-lg">Max Temp: {Math.round(main.temp_max)* 9 / 5 + 32}&deg;F</p>
            </div>
        </div>
    )

}
export default DetailsCardfahrenheit